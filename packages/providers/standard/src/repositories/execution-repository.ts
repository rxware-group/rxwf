...
export function createStandardExecutionRepository(db: StandardDatabase) {
  return {
    ...
    async updateExecutionStatus(executionId: string, status: string): Promise<void> {
      ...
      if (status === 'success' || status === 'failed') patch.finishedAt = now;
      if (status === 'waiting') patch.finishedAt = undefined;
      await db
