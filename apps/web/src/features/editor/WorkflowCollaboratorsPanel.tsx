...
interface WorkflowCollaboratorsPanelProps {
  workflowId: string;
  readOnly?: boolean;
}

export function WorkflowCollaboratorsPanel({
  workflowId,
  readOnly = false,
}: WorkflowCollaboratorsPanelProps) {
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api.workflows.listCollaborators(workflowId);
  ...
  const tableCollaborators = useMemo(
    () =>
      collaborators.filter(
        (collaborator) =>
          !creatorUserId || collaborator.userId !== creatorUserId,
      ),
    [collaborators, creatorUserId],
  );
  ...
  const openAddModal = async () => {
    ...
    try {
      const users = await api.workflows.listCollaboratorCandidates(workflowId);
  ...
  const saveCollaborators = async (
    nextCollaborators: WorkflowCollaborator[],
  ) => {
    setSaving(true);
    setError(null);
    try {
      ...
      const data = await api.workflows.updateCollaborators(workflowId, payload);
      setCreatorUserId(data.creatorUserId);
      setCreatorEmail(data.creatorEmail);
      setCollaborators(data.collaborators);
    } catch (e) {
  ...
  return (
    <section className="workflow-collaborators-panel">
      <div className="workflow-collaborators-toolbar">
        <p className="hint">
          {t(labels, 'editor.collaboratorsLead')}{' '}
          <Link to="/settings/roles">{t(labels, 'editor.viewRoles')}</Link>
        </p>
        {!readOnly && (
          <button type="button" className="btn-secondary" onClick={() => void openAddModal()}>{t(labels, 'editor.addCollaborator')}</button>
        )}
      </div>
      ...
      <table className="data-table">
          ...
          <tbody>
            {creatorUserId && (
              <tr>
                <td>{creatorEmail ?? creatorUserId}</td>
                <td>
              ...
              <tr key={collaborator.userId}>
                ...
                {!readOnly && (
                  <td>
                    ...
                    >{t(labels, 'common.remove')}</button>
                  </td>
                )}
              </tr>
            ))}
            {!creatorUserId && tableCollaborators.length === 0 && (
              <tr>
                <td colSpan={readOnly ? 2 : 3} className="hint">{t(labels, 'editor.noCollaborators')}</td>
              </tr>
            )}
          </tbody>
        </table>
